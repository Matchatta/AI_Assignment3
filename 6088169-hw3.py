...
def hillclimb_sideway(env, agent, max_iters=10000, sideway_limit=10):
    ...
    for __ in range(max_iters):
        ...
        history.append(rewards[best_i])
        if rewards[best_i] <= cur_r:
            while sideway_counter <= sideway_limit:
                neighbor = neighbors[best_i]
                ...
                if rewards[best_i] <= cur_r:
                    sideway_counter += 1
                    if rewards[best_i] == cur_r:
                        neighbors = neighbors[best_i].neighbors()
                        _n = []
                        for a in neighbors:
                            if a not in explored:
                                _n.append(a)
...
def simulated_annealing(env, agent, init_temp=25.0, temp_step=-0.1, max_iters=10000):
    ...
    for __ in range(max_iters):
        ...
        rewards = simulate(env, neighbors)
        high = len(neighbors)
        i = np.random.randint(0, high)
        history.append(rewards[i])
        current_temp = current_temp + temp_step
        if current_temp <= 0.0:
            return cur_agent, history
        else:
            next_agent = neighbors[i]
            deltaE = rewards[i] - cur_r
            if deltaE > 0:
                cur_agent = next_agent
                cur_r = rewards[i]
            else:
                probability = np.random.uniform(0, 1)
                exp = np.exp(deltaE/current_temp)
                if probability < exp:
                    cur_agent = next_agent
                    cur_r = rewards[i]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        #agent, history = hillclimb_sideway(env, agent, sideway_limit=10)
